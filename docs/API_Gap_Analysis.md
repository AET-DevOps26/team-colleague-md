...
| AI Digest as a post `type` | Epic 3 / P1 | The AI Digest is a genai-generated special post (moved here from recommendation-service). Schema adds `posts.type` (`NORMAL`/`DIGEST`). **OpenAPI gap remains**: (1) expose `type` field on `PostResponse`; (2) add `type` query param to `GET /api/v1/posts` so the Digest Management UI can fetch `?type=DIGEST` to list past digests. Frontend currently uses mock data until this endpoint is available. **Note on `eventCount`**: genai-service returns `eventCount` (number of digest events) but `posts` table has no such column. Decided: `eventCount` is not shown on the digest list card (only `read_time_minutes` is shown there); `eventCount` is only relevant on the full digest detail page, where it can be derived from the digest post's structured Markdown content. No new column needed. |
