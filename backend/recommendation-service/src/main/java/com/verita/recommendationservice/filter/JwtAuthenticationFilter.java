package com.verita.recommendationservice.filter;
 ...
 * an absent or invalid token is ignored (the context is left/cleared) and the request continues, so
 * public endpoints serve guests and authorization rules alone enforce protected endpoints.
 */
@Component
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        if (header != null && header.startsWith("Bearer ")) {
            try {
                ...
                // Protected endpoints are then rejected by the authorization rules (401 via
                // SecurityErrorHandler); public endpoints proceed anonymously.
