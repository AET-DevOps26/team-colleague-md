...
const colors = {
  ...
  bg: {
    ...
    surface:  "#F9F9F9",                  // --bg-surface   (card / panel background — legacy framed cards)
    elevated: "#F0F0F0",                  // --bg-elevated  (input / hover fill)
    paper:    "oklch(97% 0.012 80)",      // --bg-paper     (Home v3 cream block: text-only post card, digest history card)
  },
...
const semanticTokens = {
  colors: {
    ...
    "paper-bg":      { default: colors.bg.paper },     // Home v3 cream block (text-only cards, digest history)
...
// Home v3 introduces 19px (card title) and 32px (brand wordmark) named slots
const fontSizes = {
  "2xs":       "10px",
  xs:          "11.5px",
  sm:          "12px",
  ...
  lg:          "15px",
  xl:          "17px",
  "card-title":"19px",   // Home v3 card title (frameless cards: image + text)
  "2xl":       "20px",
  "3xl":       "24px",
  "4xl":       "28px",
  wordmark:    "32px",   // Home v3 sidebar wordmark (lead V renders at 38px via component override)
  "5xl":       "36px",
};
...
// ── Radii ─────────────────────────────────────────────────────
// --radius-card: 12px (Home v3 cover + cream block)  --radius-btn: 8px
const radii = {
  none:  "0",
  sm:    "4px",
  md:    "7px",
  lg:    "8px",     // --radius-btn
  xl:    "12px",    // --radius-card (Home v3: cover image, cream text block)
  "2xl": "14px",    // modal
  full:  "9999px",
};
...
// ── Sizes ─────────────────────────────────────────────────────
// Named layout constants — Home v3 sidebar widened to 240px to host the 32px wordmark
const sizes = {
  sidebar:           "240px",  // --sidebar-w (Home v3 full-width sidebar)
  "sidebar-hover":   "180px",  // hover-expanded sidebar (Post Detail/Editor/Digest/Profile/Admin) — fits 32px wordmark
...
const components = {
  ...
  Card: {
    // Home v3: cards are frameless by default — the differentiation between
    // image / text / digest cards is via the colored block (cover photo,
    // cream pull-quote panel, dark digest panel), not via card chrome.
    // Pick a variant explicitly for legacy framed cards or v3 text/digest cards.
    baseStyle: {
      container: {
        bg:           "transparent",
        border:       "none",
        borderRadius: "xl",      // --radius-card = 12px (still applies to inner colored blocks)
        boxShadow:    "none",
        padding:      0,
      },
    },
    variants: {
      // Legacy framed card (white surface + hairline border) — Settings panels, modals, etc.
      framed: {
        container: {
          bg:           "bg.surface",
          border:       "1px solid",
      ...
      // Home v3 text-only post card / digest history card (warm cream pull-quote block)
      paper: {
        container: {
          bg:           "paper-bg",
          borderRadius: "xl",
      ...
      // Home v3 digest highlight (dark curator card)
      digest: {
        container: {
          bg:           "fg",
          color:        "page-bg",
