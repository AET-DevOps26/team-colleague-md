...
![Signing in, browsing the personalized feed, opening a post, and revealing its AI-generated summary.](docs/assets/demo/hero-core-loop-1.5x.webp)

- Sign in against the real user-service — the access token is held in memory only, never in `localStorage`
- The **For you** feed ranks seeded posts per reader ([ADR-0003](docs/adr/0003-feed-ranking-and-input-sourcing.md))
- Open a post and expand **AI summary** — generated by the GenAI service, not written by hand

### The daily digest

![Opening the digest archive and reading a personalized daily digest.](docs/assets/demo/ai-daily-digest-1.5x.webp)

- One digest per day, the reader's own badged *Personalized* against the community ones
- Assembled from external sources (Hugging Face, GNews, GitHub), then written by the LLM
- A standalone entity with its own reader and read time ([ADR-0019](docs/adr/0019-standalone-digest-entity.md))

### Authoring

![Writing a post with a live Markdown preview rendering headings, lists, and KaTeX math.](docs/assets/demo/post-editor-markdown-1.5x.webp)

- Live Markdown preview beside the editor
- KaTeX math renders inline and in display blocks
- Nothing here is saved — the clip can be re-recorded without touching seeded data

### Admin: switching the model behind the AI

![An admin switching the LLM provider and model, re-summarizing a post, and reading the new summary.](docs/assets/demo/admin-genai-ops-1.5x.webp)

- Pick the LLM provider and model at runtime; `active:` is read back **from GenAI**, so it is proof the switch landed
- Re-summarize a real post and watch the job go `PENDING` → `COMPLETED` against a live LLM
- Open the post and read the summary that just came back
- The browser never calls GenAI directly: content-service fronts it with an admin JWT and forwards over an internal service-token channel ([ADR-0020](docs/adr/0020-runtime-llm-selection-and-admin-ops.md), [ADR-0007](docs/adr/0007-internal-service-authentication.md))

> Recorded with `npm run demo:record` and encoded by [`scripts/make-demo-clips.sh`](scripts/make-demo-clips.sh),
> which can re-time them to any speed (`SPEED=1` for the recorded pace). See
> [Frontend Testing](docs/testing/Frontend_Testing.md#demo-recordings).
